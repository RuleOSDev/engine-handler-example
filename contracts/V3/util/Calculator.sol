...
contract Calculator {
    ...
    uint256 constant CALCULATOR_OPT_PLUS = 10**OPT_COUNT + 1;// +
    uint256 constant CALCULATOR_OPT_MINUS = 10**OPT_COUNT + 2;// -
    uint256 constant CALCULATOR_OPT_MULTIPLY = 10**OPT_COUNT + 3;// *
    uint256 constant CALCULATOR_OPT_DIVIDE = 10**OPT_COUNT + 4;// /
    uint256 constant CALCULATOR_OPT_POWER = 10**OPT_COUNT + 5;// ^
    uint256 constant CALCULATOR_OPT_FACTORIAL = 10**OPT_COUNT + 6;// !
    uint256 constant CALCULATOR_OPT_SQRT = 10**OPT_COUNT + 7;// g  square root
    uint256 constant CALCULATOR_OPT_LN = 10**OPT_COUNT + 8;// l
    uint256 constant CALCULATOR_OPT_LOG2 = 10**OPT_COUNT + 9;// o
    uint256 constant CALCULATOR_OPT_SIN = 10**OPT_COUNT + 10;// s
    uint256 constant CALCULATOR_OPT_COS = 10**OPT_COUNT + 11;// c
    uint256 constant CALCULATOR_OPT_TAN = 10**OPT_COUNT + 12;// t

    using StackUtil for StackUtil.Stack;

    function math(int256[] memory list2) public view returns(int256){
        StackUtil.Stack memory stack;
        stack.items = new int256[](list2.length);

        uint256 indexYN = 0;

        for (uint i = 0; i < list2.length; i++) {
            if (isNumber(list2[i])) {
                console.log("number",uint(list2[i]));
                stack.push(list2[i]);
                console.log("number --- 1");
                //                }
            } else if (isOperator(list2[i])){
                console.log("operator",uint(list2[i]));
                int256 res = 0;
                if (list2[i] == int(CALCULATOR_OPT_PLUS)) {
                    int256 num2 = stack.pop();//parseFloat
                    int256 num1 = stack.pop();//parseFloat
                    res = num1 + num2;
                } else if (list2[i] == int(CALCULATOR_OPT_MINUS)) {
                    int256 num2 = stack.pop();
                    int256 num1 = stack.pop();
                    res = num1 - num2;
                } else if (list2[i] == int(CALCULATOR_OPT_MULTIPLY)) {
                    int256 num2 = stack.pop();
                    int256 num1 = stack.pop();
                    res = num1 * num2;
                } else if (list2[i] == int(CALCULATOR_OPT_DIVIDE)) {
                    int256 num2 = stack.pop();
                    int256 num1 = stack.pop();
                    if (num2 != 0){
                        res = num1 / num2;
                        ...
                        indexYN = 1;
                        require(false,"calculator divide 0");
                    }
                } else if (list2[i] == int(CALCULATOR_OPT_POWER)) {
                    int256 num2 = stack.pop();
                    int256 num1 = stack.pop();
                    res = int(uint(num1)**uint(num2));
                } else if (list2[i] == int(CALCULATOR_OPT_FACTORIAL)) {
                    int256 num1 = stack.pop();
                    if (num1 == 0 || num1 == 1){
                        res = 1;
                        ...
                        require(false,"calculator ! must integer");
                        indexYN = 1;
                    }
                } else if (list2[i] == int(CALCULATOR_OPT_SQRT)) {
                    int256 num1 = stack.pop();
                    res = int(sqrt(uint(num1)));
                } else if (list2[i] == int(CALCULATOR_OPT_LN)) {

                } else if (list2[i] == int(CALCULATOR_OPT_LOG2)) {
                    int256 num1 = stack.pop();
                    if (num1 > 0){
                        res = int(log2(uint(num1)));
                    } else {
                        require(false,"calculator log x > 0");
                        indexYN = 1;
                    }
                } else if (list2[i] == int(CALCULATOR_OPT_SIN)) {
                    int256 num1 = stack.pop();
                    res = sin(uint16(uint256(num1)));
                } else if (list2[i] == int(CALCULATOR_OPT_COS)) {
                    int256 num1 = stack.pop();
                    res = cos(uint16(uint256(num1)));
                } else if (list2[i] == int(CALCULATOR_OPT_TAN)) {
                    int256 num1 = stack.pop();
                    if (cos(uint16(uint(num1))) != 0){
                        res = tan(uint16(uint(num1)));
                    } else {
                        require(false,"calculator tan x not be +-(PI/2 + k*PI)");
                        indexYN = 1;
                    }
                }
                stack.push(res);
    ...
    function adv(int f) internal pure returns(int){
        int result = 0;
        if(f == int(CALCULATOR_OPT_PLUS)) result = 1;
        else if(f == int(CALCULATOR_OPT_MINUS)) result = 1;
        else if(f == int(CALCULATOR_OPT_MULTIPLY)) result = 2;
        else if(f == int(CALCULATOR_OPT_DIVIDE)) result = 2;
        else if(f == int(CALCULATOR_OPT_POWER)) result = 3;
        else if(f == int(CALCULATOR_OPT_FACTORIAL)) result = 4;
        else if(f == int(CALCULATOR_OPT_SQRT)) result = 4;
        else if(f == int(CALCULATOR_OPT_LN)) result = 4;
        else if(f == int(CALCULATOR_OPT_LOG2)) result = 4;
        else if(f == int(CALCULATOR_OPT_SIN)) result = 4;
        else if(f == int(CALCULATOR_OPT_COS)) result = 4;
        else if(f == int(CALCULATOR_OPT_TAN)) result = 4;
        return result;
    }
